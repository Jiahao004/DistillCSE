...
from distillcse.models_distill_calibrate import RobertaForCL, BertForCL
from distillcse.trainers_distill_calibrate import CLTrainer
...
class ModelArguments:
    ...
    distill_temp1: float = field(
        default=0.025,
        metadata={"help": "the distillation temperature for student"}
    )
    distill_temp2: float = field(
        default=0.0125,
...
class OurTrainingArguments(TrainingArguments):
    ...
    dev_file:str=field(
        default=None,
        metadata={"help":"if use external dev_file for development."}
    )

    stopping_steps:int=field(
        default=None,
        metadata={"help":"stop training at certain updating steps"}
    )
    n_gpu_for_training:int=field(
        default=1,
        metadata={"help":"the number of gpu for training."}
    )
    tt: float=field(
        default=0,
        metadata={"help":""}
...
def main():
    ...
    data_collator = default_data_collator if data_args.pad_to_max_length else OurDataCollatorWithPadding(tokenizer, padding="max_length", max_length=data_args.max_seq_length)
    ...
    if model_args.distill_teacher is not None:
        #TODO:DEBUG
        # model_args.distill_teacher = "princeton-nlp/unsup-simcse-bert-base-uncased princeton-nlp/unsup-simcse-bert-large-uncased".split()
        teachers = nn.ModuleList([AutoModel.from_pretrained(p).eval() for p in model_args.distill_teacher])

        if training_args.n_gpu==1:
            for i in range(len(teachers)):
                teachers[i] = teachers[i].to("cuda")
        else:
            for i in range(len(teachers)):
                device = torch.device(f"cuda:{i%(training_args.n_gpu -training_args.n_gpu_for_training)+training_args.n_gpu_for_training}")
                teachers[i] = teachers[i].to(device)
